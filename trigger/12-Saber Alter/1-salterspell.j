scope salterspell initializer init
        ...
        if count <= 15 then
            ...
            loop 
                ...
                if FilterGeneral(u, gUnit) and not IsUnitInGroup(gUnit, g) then 
                    call DamageUnit(u, gUnit, GetHeroStr(u, true) + 90 * GetUnitAbilityLevel(u, 'A0JC'))
            ...
            if iterator >= 3 then
                ...
                call Effect("war3mapImported\\getsugatrap.mdl", x, y, 0, a, - 25, 0, 2.5, 1, 255, 255, 255, 255)
    ...
    private function period2 takes nothing returns nothing
        ...
        if count == 165 then
            call Effect("war3mapImported\\akiha claw black.mdl", GetUnitX(t) , GetUnitY(t) , GetUnitFlyHeight(u) / 2, a, -45, 0, 2, 1.5, 255, 255, 255, 255)
    ...
    private function period3 takes nothing returns nothing
        ...
        if count >= 30 and count <= 110 then
            set x = PolarX(GetUnitX(t) , 600 / 80, a)
            set y = PolarY(GetUnitY(t) , 600 / 80, a)
            call SetUnitXY(t, x, y, 2)
            set iterator = iterator + 1
            call SaveInteger(ht, zid, 1, iterator)
            if iterator >= 10 then
    ...
    private function period5 takes nothing returns nothing
        ...
        if count >= 80 and count <= 120 then
            set e = LoadEffectHandle(ht, zid, 100)
            set x = PolarX(GetEffectX(e) , 2000 / 40, a)
            set y = PolarY(GetEffectY(e) , 2000 / 40, a)
            call SetEffectXY(e, x, y, 1)
