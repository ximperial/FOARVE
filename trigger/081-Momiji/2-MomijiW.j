//TESH.scrollpos=0
//TESH.alwaysfold=0
function Trig_MomijiW_Func001 takes nothing returns nothing
    ...
    if count == 1 then
        call SoundStart("war3mapImported\\Momiji_2.mp3")
        call SoundStart("war3mapImported\\Momiji_2-1.mp3")
    ...
    if count <= 20 then
        ...
        if iterator >= 3 then
            ...
            call Effect("war3mapimported\\tx-smoke1.mdx", GetUnitX(c), GetUnitY(c), 25, a, 0, 0, 1.5, 1, 255, 255, 255, PercentTo255(80))
        endif
    endif
    if count == 20 then
    ...
    if count == 40 then
        call SoundStart("war3mapImported\\Momiji_2-3.mp3")
    ...
    if count == 50 then 
        ...
        call DamageUnit(c, t, (1 + GetUnitAbilityLevel(c, 'A0HG')) * GetHeroAgi(c, true))
...
function Trig_MomijiW_Actions takes nothing returns nothing
    local timer z
    ...
    if GetSpellAbilityId() == 'A0HG' then 
        ...
        call SaveInteger(ht, GetHandleId(z) , 1, 0)
        call TimerStart(z, 0.02, true, function Trig_MomijiW_Func001)
    endif
    set z = null
    set c = null
    set t = null
...
//===========================================================================
function InitTrig_MomijiW takes nothing returns nothing
    set gg_trg_MomijiW = CreateTrigger()
    call TriggerRegisterAnyUnitEventBJ(gg_trg_MomijiW, EVENT_PLAYER_UNIT_SPELL_EFFECT)
    call TriggerAddAction(gg_trg_MomijiW, function Trig_MomijiW_Actions)
endfunction