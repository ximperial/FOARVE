scope vegetaspell initializer init
        ...
        if count <= 15 then
            ...
            loop
                ...
                if FilterGeneral(u, gUnit) and not IsUnitInGroup(gUnit, g) then
                    call DamageUnit(u, gUnit, GetHeroStr(u, true) + 90 * GetUnitAbilityLevel(u, 'A04F'))
        ...
        if count <= 150 then
            ...
            if iterator >= 5 then
                ...
                if count >= 20 then
                    ...
                    call SetSpecialEffectAnimation(bj_lastCreatedEffect, "death")
                    call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.75)
                    call Effect("war3mapImported\\hiteffect07bluea.mdl", GetUnitX(t) , GetUnitY(t) , 50, randomAngle() , 0, 0, 5, 1.5, 255, 255, 255, 255)
                endif
            endif
        endif
        if count == 150 then
    ...
    private function period4 takes nothing returns nothing
        ...
        if count == 40 then
            ...
            loop
                set gUnit = GroupForEachUnit(gGroup)
                exitwhen gUnit == null
                if FilterGeneral(u, gUnit) and IsUnitInConeEx(gUnit, GetUnitX(u) , GetUnitY(u) , a, 140) then
    ...
    private function period6_2 takes nothing returns nothing
        ...
        call SetSpecialEffectHeight(e, h)
    ...
    private function period6 takes nothing returns nothing
        ...
        if count >= 160 and count <= 190 then
            ...
            call SetSpecialEffectHeight(e, GetEffectHeight(e) + 15)
        ...
        if count == 200 then
            ...
            call SetUnitInvulnerable(u, false)
            call SetUnitInvulnerable(t, false)
            call DamageUnit(u, t, (8 + B2I(GetUnitAbilityLevel(u, 'B00T') > 0) * 2) * GetHeroStr(u, true))
            call VisionArea(GetOwningPlayer(u) , 1200, 3, GetUnitX(t) , GetUnitY(t))
            call IssueTargetOrderById(u, 851983, t)
            call IssueImmediateOrderById(t, 851972)
            call FlushChildHashtable(ht, GetHandleId(z))
