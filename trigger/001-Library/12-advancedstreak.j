scope advancedstreak initializer init
    ...
    private function KillSystem takes nothing returns nothing
        ...
        if IsUnitHero(t) and not IsUnitIllusion(t) then
            ...
            loop
                exitwhen i > 4
                if IsUnitInGroup(t, locktour[i]) then
                    call GroupRemoveUnit(locktour[i], t)
                endif
                set i = i + 1
            endloop
            if TourType == 1 then
                if p2 == Player(1) then
                    call SetPlayerAllianceStateBJ( p2, Player(2), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(3), bj_ALLIANCE_ALLIED_VISION )
                elseif p2 == Player(2) then
                    call SetPlayerAllianceStateBJ( p2, Player(1), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(3), bj_ALLIANCE_ALLIED_VISION )
                elseif p2 == Player(3) then
                    call SetPlayerAllianceStateBJ( p2, Player(1), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(2), bj_ALLIANCE_ALLIED_VISION )
                elseif p2 == Player(5) then
                    call SetPlayerAllianceStateBJ( p2, Player(6), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(7), bj_ALLIANCE_ALLIED_VISION )
                elseif p2 == Player(6) then
                    call SetPlayerAllianceStateBJ( p2, Player(5), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(7), bj_ALLIANCE_ALLIED_VISION )
                elseif p2 == Player(7) then
                    call SetPlayerAllianceStateBJ( p2, Player(5), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(6), bj_ALLIANCE_ALLIED_VISION )
                elseif p2 == Player(9) then
                    call SetPlayerAllianceStateBJ( p2, Player(10), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(11), bj_ALLIANCE_ALLIED_VISION )
                elseif p2 == Player(10) then
                    call SetPlayerAllianceStateBJ( p2, Player(9), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(11), bj_ALLIANCE_ALLIED_VISION )
                elseif p2 == Player(11) then
                    call SetPlayerAllianceStateBJ( p2, Player(9), bj_ALLIANCE_ALLIED_VISION )
                    call SetPlayerAllianceStateBJ( p2, Player(10), bj_ALLIANCE_ALLIED_VISION )
                endif
            endif
            if TourType == 2 then
                if IsUnitAlly(t, Player(0)) then
                    call SetPlayerAllianceStateBJ( p2, Player(5), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(6), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(7), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(9), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(10), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(11), bj_ALLIANCE_UNALLIED )
                elseif IsUnitAlly(t, Player(4)) then
                    call SetPlayerAllianceStateBJ( p2, Player(1), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(2), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(3), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(9), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(10), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(11), bj_ALLIANCE_UNALLIED )
                elseif IsUnitAlly(t, Player(8)) then
                    call SetPlayerAllianceStateBJ( p2, Player(1), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(2), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(3), bj_ALLIANCE_UNALLIED )
                    call SetPlayerAllianceStateBJ( p2, Player(5), bj_ALLIANCE_UNALLIED )
            ...
            if u != null then
                if RectContainsCoords(gg_rct_Boss_Arena, GetUnitX(t), GetUnitY(t)) and not bossKiller[pid2] then
                    set bossKiller[pid2] = true
                    call AdjustPlayerStateBJ(3000, p2, PLAYER_STATE_RESOURCE_GOLD)
                endif
