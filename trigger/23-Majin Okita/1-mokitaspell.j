scope mokitaspell initializer init
    ...
    private function period1 takes nothing returns nothing
        ...
        if count <= 15 then
            ...
            call SetSpecialEffectHeight(e, 150)
            ...
            loop
                ...
                if FilterGeneral(u, gUnit) and not IsUnitInGroup(gUnit, g) then
                    call DamageUnit(u, gUnit, GetHeroAgi(u, true) + 90 * GetUnitAbilityLevel(u, 'A04R'))
    ...
    private function period4 takes nothing returns nothing
        ...
        if count == 130 then
            ...
            call SetUnitTimeScale(u, 1)
            call Effect("war3mapImported\\amx (345)_white.mdl", GetUnitX(t), GetUnitY(t), 150, a, 0, 0, 4, 2, 255, PercentTo255(20), PercentTo255(20), 255)
            call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.5)
        endif
        if count == 140 then
            call SetUnitTimeScale(u, 1)
    ...
    private function period5 takes nothing returns nothing
        ...
        if count >= 150 and count <= 170 then
            ...
            call SetSpecialEffectHeight(e, 100 + GetAxisZ(x, y))
            set iterator = iterator + 1
            call SaveInteger(ht, zid, 1, iterator)
            if iterator >= 5 then
                call SaveInteger(ht, zid, 1, 0)
        ...
        if count >= 170 and count <= 240 then
            ...
            if iterator >= 5 then
                ...
                call CinematicFilterGenericEx(GetOwningPlayer(u), 1, BLEND_MODE_BLEND, "ImageSpell\\OkitaAlter" + I2S(i) + ".blp", 255, 255, 255, 255, 255, 255, 255, 0)
                call CinematicFilterGenericEx(GetOwningPlayer(t), 1, BLEND_MODE_BLEND, "ImageSpell\\OkitaAlter" + I2S(i) + ".blp", 255, 255, 255, 255, 255, 255, 255, 0)
            endif
        endif
        if count == 180 then
    ...
    private function period6 takes nothing returns nothing
        ...
        if count == 20 then
            ...
            call ShakeCamera(6, GetOwningPlayer(t), 10)
        ...
        if count == 350 then
            ...
            call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.75)
            call Effect("war3mapImported\\OkitaAlterEff5.mdl", GetUnitX(t), GetUnitY(t), 100, GetUnitFacing(u), 0, 0, 5, 1.5, 255, 255, 255, 255)
            call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.75)
        endif
    ...
    private function period7 takes nothing returns nothing
        ...
        if count >= 40 and count <= 50 or count >= 55 and count <= 65 or count >= 70 and count <= 80 then
            ...
            call SetEffectXY(e, x, y, 0)
            call SetSpecialEffectHeight(e, 150 + GetAxisZ(x, y))
            set e = LoadEffectHandle(ht, zid, 101)
            call SetEffectXY(e, x, y, 0)
            call SetSpecialEffectHeight(e, 150 + GetAxisZ(x, y))
            set iterator = iterator + 1
