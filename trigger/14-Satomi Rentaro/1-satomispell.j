scope satomispell initializer init
    ...
    private function period1 takes nothing returns nothing
        ...
        if count == 1 then
            ...
            call Effect("war3mapImported\\BY_Wood_Eff_Ord_DanGe_Dus_Kuosan_1_2_2.mdl", GetUnitX(u), GetUnitY(u), 0, GetUnitFacing(u), 0, 0, 1.5, 1, 255, 255, 255, 255)
        endif
        if count == 10 then
            call ShakeCamera(1, GetOwningPlayer(u), 5)
            call SetUnitXY(u, x, y, 1)
            call Effect("war3mapImported\\BY_Wood_Eff_Ord_DanGe_Wid_ChongFengQiLiu.mdl", GetUnitX(u), GetUnitY(u), 0, GetUnitFacing(u), 0, 0, 1.5, 1, 255, 255, 255, 255)
            call Effect("war3mapImported\\BY_Wood_Eff_Ord_YeYe_Eat_DiBanZhaKai.mdl", GetUnitX(u), GetUnitY(u), 0, randomAngle(), 0, 0, 1.3, 1, 255, 255, 255, 255)
            call Effect("war3mapImported\\BY_Wood_Effect_Order_DanGe_DaoGuang_BaoZha_2_2_2.mdl", GetUnitX(u), GetUnitY(u), 0, randomAngle(), 0, 0, 1.1, 1, 255, 255, 255, 255)
            call Effect("war3mapImported\\Dls1.mdl", GetUnitX(u), GetUnitY(u), 0, randomAngle(), 0, 0, 1.1, 1, 255, 255, 255, 255)
            call GroupEnumUnitsInRange(gGroup, GetUnitX(u), GetUnitY(u), 500, null)
            loop
                set gUnit = GroupForEachUnit(gGroup)
                exitwhen gUnit == null
                if FilterGeneral(u, gUnit) then
                    call DamageUnit(u, gUnit, GetHeroStr(u, true) + 90 * GetUnitAbilityLevel(u, 'A01F'))
                endif
            endloop
    ...
    private function period4 takes nothing returns nothing
        ...
        if count == 100 then
            ...
            call Effect("war3mapImported\\EarthDetonation.mdl", GetUnitX(t), GetUnitY(t), 0, randomAngle(), 0, 0, 2.5, 1, 255, 255, 255, 255)
            call Effect("war3mapImported\\SZBYAFB2.mdl", GetUnitX(t), GetUnitY(t), 0, randomAngle(), 0, 0, 2.2, 1, 255, 255, 255, 255)
    ...
    private function period6 takes nothing returns nothing
        ...
        if count == 160 then 
            ...
            call SetUnitInvulnerable(u, false)
            call SetUnitInvulnerable(t, false)
            call DamageUnit(u, t, (8 + B2I(GetUnitAbilityLevel(u, 'A01K') > 0) * 2) * GetHeroStr(u, true))
            call VisionArea(GetOwningPlayer(u), 1200, 3, GetUnitX(t), GetUnitY(t))
            call IssueTargetOrderById(u, 851983, t)
            call IssueImmediateOrderById(t, 851972)
            call DestroyEffect(LoadEffectHandle(ht, GetHandleId(z), 10))
