//TESH.scrollpos=0
//TESH.alwaysfold=0
function Trig_NanayaT_Func001 takes nothing returns nothing
    local timer z = GetExpiredTimer()
    local unit c = LoadUnitHandle(ht, GetHandleId(z) , 0)
    local unit t = LoadUnitHandle(ht, GetHandleId(z) , 1)
    local integer count = LoadInteger(ht, GetHandleId(z) , 0)
    local integer iterator = LoadInteger(ht, GetHandleId(z) , 1)
    local real x
    local real y
    local real a = Angle(GetUnitX(c) , GetUnitY(c) , GetUnitX(t) , GetUnitY(t))
    local real dist = Distance(GetUnitX(c) , GetUnitY(c) , GetUnitX(t) , GetUnitY(t))
    local integer i = 0
    local effect e = LoadEffectHandle(ht, GetHandleId(z) , 3)
    local effect e2 = LoadEffectHandle(ht, GetHandleId(z) , 4)
    local real dmg
    set count = count + 1
    call SaveInteger(ht, GetHandleId(z) , 0, count)
    call PauseUnit(c, true)
    call SetUnitInvulnerable(c, true)
    call PauseUnit(t, true)
    call SetUnitInvulnerable(t, true)
    if count == 1 then
        call SoundStart("war3mapImported\\Nanaya_5.mp3")
        call SaveUnitHandle(ht, GetHandleId(z) , 2, CreateUnit(Player(12) , 'H00D', GetUnitX(c) , GetUnitY(c) , a)) 
        call SetUnitX(c, PolarX(GetUnitX(t) , - 1000, a))
        call SetUnitY(c, PolarY(GetUnitY(t) , - 1000, a))
        call DestroyEffect(AddSpecialEffectTarget("Abilities\\Spells\\Human\\Invisibility\\InvisibilityTarget.mdl", c, "right hand"))
        call Effect("war3mapimported\\devilslam.mdx", GetUnitX(c) , GetUnitY(c) , 25, a, 0, 0, 1.2, 1, 255, 255, 255, PercentTo255(80))
        call Effect("war3mapimported\\wind2.mdx", GetUnitX(c) , GetUnitY(c) , 25, a, 0, 0, 1.5, 1, 255, 255, 255, PercentTo255(60))
        call Effect("war3mapimported\\wavejojo.mdx", GetUnitX(c) , GetUnitY(c) , 25, a, 0, 0, 1, 1, 255, 255, 255, PercentTo255(80))
        call SetUnitAnimationByIndex(c, 9)
        call SetUnitTimeScale(c, 0.75)
    endif
    if count == 20 then
        call Effect("war3mapimported\\wavejojo.mdx", GetUnitX(c) , GetUnitY(c) , 120, a, - 90, 0, 1, 1, 255, 255, 255, 255)
        call Effect("war3mapimported\\wavejojo.mdx", GetUnitX(c) , GetUnitY(c) , 25, a, 0, 0, 1, 1, 255, 255, 255, PercentTo255(80))
        call Effect("war3mapimported\\wind2.mdx", GetUnitX(c) , GetUnitY(c) , 120, a, - 90, 0, 1.5, 1, 255, 255, 255, 255)
        call Effect("war3mapimported\\qyddd.mdx", GetUnitX(c) , GetUnitY(c) , 100, a, 0, 0, 1.5, 1, 255, 255, 255, 255)
        call SaveEffectHandle(ht, GetHandleId(z) , 3, bj_lastCreatedEffect)
        set e = bj_lastCreatedEffect
        call Effect("war3mapimported\\Big_ICM_Blue.mdx", GetUnitX(c) , GetUnitY(c) , 100, a, 0, 0, 1, 1, 255, 255, 255, 255)
        call SaveEffectHandle(ht, GetHandleId(z) , 4, bj_lastCreatedEffect)
        set e2 = bj_lastCreatedEffect
    endif
    if count >= 20 and count <= 40 then
        call SetEffectX(e, PolarX(GetEffectX(e) , dist / 20, a))
        call SetEffectY(e, PolarY(GetEffectY(e) , dist / 20, a))
        call SetSpecialEffectZ(e, 100 + GetAxisZ(GetEffectX(e) , GetEffectY(e)))
        call SetEffectX(e2, PolarX(GetEffectX(e2) , dist / 20, a))
        call SetEffectY(e2, PolarY(GetEffectY(e2) , dist / 20, a))
        call SetSpecialEffectZ(e2, 100 + GetAxisZ(GetEffectX(e2) , GetEffectY(e2)))
        set iterator = iterator + 1
        call SaveInteger(ht, GetHandleId(z) , 1, iterator)
        if iterator >= 3 then
            call SaveInteger(ht, GetHandleId(z) , 1, 0)
            call Effect("war3mapimported\\wind2.mdx", GetUnitX(c) , GetUnitY(c) , 150, a, -90, 0, 1, 1, 255, 255, 255, 255)
        endif
    endif
    if count == 40 then 
        call SoundStart("war3mapImported\\Nanaya_5-1.mp3")
        call SetUnitX(c, PolarX(GetUnitX(t) , - 120, a))
        call SetUnitY(c, PolarY(GetUnitY(t) , - 120, a))
        call SetUnitTimeScale(c, 1.5) 
        call Effect("war3mapimported\\arcdirve02b.mdx", GetUnitX(t) , GetUnitY(t) , 40, a, 0, 0, 5, 1, 255, 255, 255, 255)
        call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.8)
        call Effect("war3mapimported\\[A]re1.mdx", GetUnitX(t) , GetUnitY(t) , 40, a, 0, 0, 5, 1, 255, 255, 255, 255)
        call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.5)
        call Effect("war3mapimported\\[A]mihawkslashhit.mdx", GetUnitX(t) , GetUnitY(t) , 40, a + 20, 0, 0, 4, 1, 255, 255, 255, 255)
        call SaveReal(ht, GetHandleId(z) , 0, a)
    endif
    if count >= 40 and count <= 50 then
        set a = LoadReal(ht, GetHandleId(z) , 0)
        call SetUnitX(c, PolarX(GetUnitX(c) , 26, a))
        call SetUnitY(c, PolarY(GetUnitY(c) , 26, a))
    endif
    if count == 50 then
        call Effect("war3mapimported\\wind2.mdx", GetUnitX(t) , GetUnitY(t) , 25, a, 0, 0, 1.5, 1, 255, 255, 255, PercentTo255(60))
        call Effect("war3mapimported\\wavejojo.mdx", GetUnitX(t) , GetUnitY(t) , 25, a, 0, 0, 1, 1, 255, 255, 255, PercentTo255(80))
        call Effect("war3mapimported\\UltiZoro.mdx", GetUnitX(t) , GetUnitY(t) , 25, a, 0, 0, 3.5, 1, 255, 255, 255, 255)
        call Effect("war3mapimported\\bloodex1.mdx", GetUnitX(t) , GetUnitY(t) , 25, a, 0, 0, 3.5, 1, 255, 255, 255, 255)
    endif
    if count == 60 then 
        call SetUnitTimeScale(c, 1)
        call PauseUnit(c, false)
        call PauseUnit(t, false)
        call SetUnitInvulnerable(c, false)
        call SetUnitInvulnerable(t, false)
        set dmg = 0.4 * GetUnitAbilityLevel(c, 'B01N')
        call DamageUnit(c, t, (dmg + 15) * GetHeroAgi(c, true))
        call VisionArea(GetOwningPlayer(c) , 1200, 3, GetUnitX(t) , GetUnitY(t))
        call IssueTargetOrderById(c, 851983, t)
        call IssueImmediateOrderById(t, 851972)
        call RemoveUnit(LoadUnitHandle(ht, GetHandleId(z) , 2))
        call FlushChildHashtable(ht, GetHandleId(z))
        call PauseTimer(z)
        call DestroyTimer(z)
    endif
    set z = null
    set c = null
    set t = null
    set e = null
    set e2 = null
endfunction

function Trig_NanayaT_Actions takes nothing returns nothing
    local timer z
    local unit c = GetTriggerUnit()
    local unit t = GetSpellTargetUnit()
    if GetSpellAbilityId() == 'A06Z' then 
        set z = CreateTimer()
        call PauseUnit(c, true)
        call SetUnitInvulnerable(c, true)
        call PauseUnit(t, true)
        call SetUnitInvulnerable(t, true)
        call SaveUnitHandle(ht, GetHandleId(z) , 0, c)
        call SaveUnitHandle(ht, GetHandleId(z) , 1, t)
        call SaveInteger(ht, GetHandleId(z) , 0, 0)
        call SaveInteger(ht, GetHandleId(z) , 1, 0)
        call TimerStart(z, 0.02, true, function Trig_NanayaT_Func001)
    endif
    set z = null
    set c = null
    set t = null
endfunction

//===========================================================================
function InitTrig_NanayaT takes nothing returns nothing
    set gg_trg_NanayaT = CreateTrigger()
    call TriggerRegisterAnyUnitEventBJ(gg_trg_NanayaT, EVENT_PLAYER_UNIT_SPELL_EFFECT)
    call TriggerAddAction(gg_trg_NanayaT, function Trig_NanayaT_Actions)
endfunction