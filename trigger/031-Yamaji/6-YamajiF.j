//TESH.scrollpos=284
//TESH.alwaysfold=0
function Trig_YamajiF_Func001 takes nothing returns nothing
    ...
    if count == 150 then
        ...
        call Effect("war3mapImported\\Genryuusai.mdl", GetUnitX(c), GetUnitY(c), 0, a, 0, 0, 1, 1.5, 255, 255, 255, PercentTo255(40))
    ...
    if count <= 170 then
        ...
        if iterator >= 10 then 
            ...
            call Effect("war3mapImported\\[A]az_axe_ef1.mdl", GetUnitX(c), GetUnitY(c), 25, randomAngle(), 0, 0, GetRandomReal(0.75, 1.2), 1, 255, 255, 255, PercentTo255(GetRandomReal(30, 70)))
            call SetSpecialEffectTimeScale(bj_lastCreatedEffect, GetRandomReal(0.4, 0.8)) 
            if count >= 80 then 
                call Effect("war3mapImported\\WindExploreEffect(Bigger).mdl", GetUnitX(c), GetUnitY(c), 300, randomAngle(), - 90, 0, GetRandomReal(1.5, 2), 1, 255, 255, 255, PercentTo255(GetRandomReal(20, 60)))
    ...
    if count == 280 then
        ...
        set x = PolarX(GetUnitX(c), - 250, a)
        set y = PolarY(GetUnitY(c), - 250, a)
        call Effect("war3mapImported\\BY_Wood_GongChengSiPai_5.mdl", x, y, 25, a, 0, 0, 1, 1, 255, 255, 255, PercentTo255(50))
        call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.8)
    endif
    if count == 290 then
        call Effect("war3mapImported\\[A]GrassPisses.mdl", GetUnitX(t), GetUnitY(t), 150, a, 0, 0, 4, 1, 255, 255, 255, 255)
        call SetSpecialEffectAnimationByIndex(bj_lastCreatedEffect, 1)
        ...
        call Effect("war3mapImported\\BY_Wood_GongChengSiPai_1.mdl", GetUnitX(t), GetUnitY(t), 150, a, - 180, 0, 2, 1, 255, 255, 255, 255)
        call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.7)
        ...
        call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.4)
        call Effect("war3mapImported\\az_hitheavy.mdl", GetUnitX(t), GetUnitY(t), 150, a, 0, 0, 2, 1, 255, 255, 255, 255)
        call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.6)
    endif
    if count == 300 then 
...
function Trig_YamajiF_Func002 takes nothing returns nothing
    local timer z = GetExpiredTimer()
    local unit c = LoadUnitHandle(ht, GetHandleId(z), 0)
    local unit t = LoadUnitHandle(ht, GetHandleId(z), 1)
    local integer count = LoadInteger(ht, GetHandleId(z), 0)
    ...
    if count <= 270 then
        ...
        if iterator == 10 or iterator == 15 then 
            ...
            call Effect("war3mapImported\\[A]az_axe_ef1.mdl", GetUnitX(c), GetUnitY(c), 25, randomAngle(), 0, 0, GetRandomReal(1, 1.35), 1, 255, PercentTo255(80), PercentTo255(50), 255)
            call SetSpecialEffectTimeScale(bj_lastCreatedEffect, GetRandomReal(0.4, 0.6))
        endif
    endif
    if count == 270 then
...
function Trig_YamajiF_Actions takes nothing returns nothing
    local timer z
    ...
    if GetSpellAbilityId() == 'A0BG' then 
        ...
        if GetUnitTypeId(c) == 'H0FV' then
            call TimerStart(z, 0.02, true, function Trig_YamajiF_Func002)
        else
            call TimerStart(z, 0.02, true, function Trig_YamajiF_Func001)
        endif
    endif
    set z = null
    set c = null
...
//===========================================================================
function InitTrig_YamajiF takes nothing returns nothing
    set gg_trg_YamajiF = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ(gg_trg_YamajiF, EVENT_PLAYER_UNIT_SPELL_EFFECT)
    call TriggerAddAction( gg_trg_YamajiF, function Trig_YamajiF_Actions )
endfunction