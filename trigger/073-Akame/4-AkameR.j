//TESH.scrollpos=0
//TESH.alwaysfold=0
function Trig_AkameR_Func001 takes nothing returns nothing
    ...
    if count >= 10 and count <= 50 then
        ...
        if iterator >= 3 then
            ...
            call Effect("war3mapimported\\QQQQQR.mdx", x, y, GetRandomReal(50, 150), a, 0, 0, 1.5, 1, 255, 255, 255, 255)
        endif
    endif
    if count == 50 then
    ...
    if count == 80 then 
        ...
        call DamageUnit(c, t, (6 + 2 * GetUnitAbilityLevel(c, 'A0FR')) * GetHeroAgi(c, true))
...
function Trig_AkameR_Actions takes nothing returns nothing
    local timer z
    ...
    if GetSpellAbilityId() == 'A0FR' then 
        ...
        call SaveInteger(ht, GetHandleId(z) , 1, 0)
        call TimerStart(z, 0.02, true, function Trig_AkameR_Func001)
    endif
    set z = null
    set c = null
    set t = null
...
//===========================================================================
function InitTrig_AkameR takes nothing returns nothing
    set gg_trg_AkameR = CreateTrigger()
    call TriggerRegisterAnyUnitEventBJ(gg_trg_AkameR, EVENT_PLAYER_UNIT_SPELL_EFFECT)
    call TriggerAddAction(gg_trg_AkameR, function Trig_AkameR_Actions)
endfunction