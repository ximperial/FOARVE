//TESH.scrollpos=51
//TESH.alwaysfold=0
function Trig_GutsE_Func001 takes nothing returns nothing
    ...
    if count == 1 then
        ...
        call SetSpecialEffectTimeScale(bj_lastCreatedEffect, 0.6)
        call Effect("Abilities\\Weapons\\BoatMissile\\BoatMissile.mdl", GetUnitX(c), GetUnitY(c), 0, a, 0, 0, 1, 1, 255, 255, 255, 255)
    ...
    if count == 35 then 
        ...
        loop 
            ...
            if FilterGeneral(c, gUnit) then 
                call DamageUnit(c, gUnit, (1.5 + 0.5 * GetUnitAbilityLevel(c, 'A0B4')) * GetHeroStr(c, true))
            endif 
        endloop
...
function Trig_GutsE_Actions takes nothing returns nothing
    local timer z
    ...
    if GetSpellAbilityId() == 'A0B4' then 
        ...
        call TimerStart(z, 0.02, true, function Trig_GutsE_Func001)
    endif
    set z = null
    set c = null
endfunction
...
//===========================================================================
function InitTrig_GutsE takes nothing returns nothing
    set gg_trg_GutsE = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ(gg_trg_GutsE, EVENT_PLAYER_UNIT_SPELL_EFFECT)
    call TriggerAddAction( gg_trg_GutsE, function Trig_GutsE_Actions )
endfunction