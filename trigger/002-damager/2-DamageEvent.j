scope damageevent initializer init
    ...
    private function action takes nothing returns nothing
        ...
        if LoadBoolean(ht, StringHash("CronusF"), GetHandleId(target)) and GetUnitTypeId(source) == 'H05Z' then
            call SaveReal(ht, StringHash("CronusF"), GetHandleId(target), LoadReal(ht, StringHash("CronusF"), GetHandleId(target)) + dmg / 0.75)
        ...
        if dmg > 0 and GetUnitAbilityLevel(source, 'A04G') > 0 and FilterGeneral(source, target) and(GetEventIsAttack() or GetEventIsRanged()) then
            ...
            call UnitAddBonus(source, BONUS_ATTACK, 4 + alvl)
            call SaveUnitHandle(ht, GetHandleId(z) , 0, source)
            call SaveInteger(ht, GetHandleId(z) , 0, 4 + alvl)
            call TimerStart(z, 30, false, function HibariE_End)
            set z = null
        endif
        ...
        set d = LoadUnitHandle(ht, StringHash("AizenD"), GetHandleId(source))
        ...
        if dmg > GetHeroPrimaryStat(source, true) * 2 and GetUnitAbilityLevel(target, 'B016') > 0 and IsUnitEnemy(source, GetOwningPlayer(d)) and IsUnitType(source, UNIT_TYPE_HERO) and GetUnitTypeId(source) != 'H046' and IsUnitAlive(source) then
        ...
        if dmg > 0 and FilterGeneral(source, target) and(UnitHasItemOfTypeBJ(source, 'I01Q') or UnitHasItemOfTypeBJ(source, 'I01R') or UnitHasItemOfTypeBJ(source, 'I04P')) then
            if GetEventIsAttack() or GetEventIsRanged() then
                if not IsAbilityOnCooldown(GetUnitAbility(source, 'A0BI')) then
                    ...
                    call UnitAttackRestart(source)
                endif
            endif
        endif
        
        if dmg > 0 and GetUnitAbilityLevel(target, 'B002') > 0 then
            call SaveReal(ht, StringHash("sky") , GetHandleId(target) , LoadReal(ht, StringHash("sky") , GetHandleId(target)) + dmg / 0.75)
            call SaveReal(ht, StringHash("sky_" + I2S(cid)), GetHandleId(target), LoadReal(ht, StringHash("sky_" + I2S(cid)), GetHandleId(target)) + dmg / 0.75)
            set d = LoadUnitHandle(ht, StringHash("sky") , GetHandleId(target))
            if d == null then
                call SaveUnitHandle(ht, StringHash("sky") , GetHandleId(target) , source)
            endif
            if source != d and LoadReal(ht, StringHash("sky_" + I2S(cid)), GetHandleId(target)) > LoadReal(ht, StringHash("sky_" + I2S(GetPlayerId(GetOwningPlayer(d)))), GetHandleId(target)) then
                call SaveUnitHandle(ht, StringHash("sky") , GetHandleId(target) , source)
            endif
            call SetEventDamage(0)
            set dmg = GetEventDamage()
        ...
        if dmg > 0 and GetUnitAbilityLevel(target, 'B01I') > 0 and IsUnitEnemy(source, GetOwningPlayer(target)) and IsUnitAlive(source) and not IsUnitTower(source) and GetUnitTypeId(source) != 'h09C' then
            if IsUnitInvulnerable(source) then
                set vdmg = dmg * 0.6 / 0.75
                if vdmg >= GetUnitCurrentLife(source) then
                    call SetUnitCurrentLife(source, GetUnitMaxLife(source) * 0.01)
                else
        ...
        if dmg > 0 and not IsUnitTower(target) and (GetEventIsAttack() or GetEventIsRanged()) and (UnitHasItemOfTypeBJ(source, 'I06N') or UnitHasItemOfTypeBJ(source, 'I06O') or UnitHasItemOfTypeBJ(source, 'I06P')) then
            ...
            if dmg > GetUnitCurrentLife(target) then
                if UnitHasItemOfTypeBJ(source, 'I06P') then
                    set vdmg = GetUnitCurrentLife(target) * 0.5
                elseif UnitHasItemOfTypeBJ(source, 'I06O') then
                    set vdmg = GetUnitCurrentLife(target) * 0.4
                elseif UnitHasItemOfTypeBJ(source, 'I06N') then
        ...
        if dmg > 0 and UserPlayer(GetOwningPlayer(source)) and target != gg_unit_H0DS_0046 then
            if dmg < GetHeroPrimaryStat(source, true) then
                call ArcingTextTag.createEx(COLOR[cid] + I2S(R2I(dmg)), target, 0.5, 1, GetOwningPlayer(source))
            else
                call ArcingTextTag.createEx(COLOR[cid] + I2S(R2I(dmg)), target, 1, 1, GetOwningPlayer(source))
            endif
        endif
