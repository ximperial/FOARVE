//TESH.scrollpos=0
//TESH.alwaysfold=0
function Trig_KianaQ_Func001 takes nothing returns nothing
    ...
    if count == 1 then
        call SoundStart("war3mapImported\\Kiana_1.mp3")
    ...
    if count <= 15 then
        ...
        loop
            ...
            if FilterGeneral(c, gUnit) and not IsUnitInGroup(gUnit, g) then
                call DamageUnit(c, gUnit, 100 * GetUnitAbilityLevel(c, 'A03T'))
        ...
        if iterator >= 2 then
            ...
            call Effect("war3mapimported\\Artillery.mdx", GetEffectX(e1), GetEffectY(e1), 25, GetSpecialEffectYaw(e1), 0, 0, 1, 1, 255, 255, 255, 255)
            call Effect("war3mapimported\\Artillery.mdx", GetEffectX(e2), GetEffectY(e2), 25, GetSpecialEffectYaw(e2), 0, 0, 1, 1, 255, 255, 255, 255)
            call Effect("war3mapimported\\Artillery.mdx", GetEffectX(e3), GetEffectY(e3), 25, GetSpecialEffectYaw(e3), 0, 0, 1, 1, 255, 255, 255, 255)
        endif
    endif
    if count == 15 then
...
function Trig_KianaQ_Actions takes nothing returns nothing
    local timer z
    ...
    if GetSpellAbilityId() == 'A03T' then
        ...
        call TimerStart(z, 0.02, true, function Trig_KianaQ_Func001)
        set z = null
    endif
    set z = null
    set c = null
...
//===========================================================================
function InitTrig_KianaQ takes nothing returns nothing
    set gg_trg_KianaQ = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ(gg_trg_KianaQ, EVENT_PLAYER_UNIT_SPELL_EFFECT)
    call TriggerAddAction( gg_trg_KianaQ, function Trig_KianaQ_Actions )
endfunction