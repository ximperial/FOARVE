scope stingspell initializer init
    ...
    private function period1 takes nothing returns nothing
        ...
        if count <= 15 then
            ...
            set dmg = B2I(GetUnitTypeId(u) == 'H0CS') * GetHeroStr(u, true)
    ...
    private function period2 takes nothing returns nothing
        ...
        if count == 80 then 
            ...
            call DamageUnit(u, t, (B2I(GetUnitTypeId(u) == 'H0CS') + 1 + GetUnitAbilityLevel(u, 'A025')) * GetHeroStr(u, true))
            if uid != 'H0CS' then
    ...
    private function period3 takes nothing returns nothing
        ...
        if count == 10 then
            ...
            if GetUnitAbilityLevel(u, 'A023') > 0 then
                ...
                if ab != null then
                    call ShowAbilityEx(u, 'A023', false)
                endif
            ...
            if GetUnitAbilityLevel(u, 'A02F') > 0 then
                ...
                if ab != null then
                    call ShowAbilityEx(u, 'A02F', false)
                endif
        ...
        if count > 10 and GetUnitAbilityLevel(u, 'B00G') == 0 and IsUnitAlive(u) then
            ...
            if GetUnitAbilityLevel(u, 'A023') > 0 then
                ...
                set ab = GetUnitAbility(u, 'A023')
                if ab != null then
                    call ShowAbilityEx(u, 'A023', true)
                endif
                if cd != 0 then
                    call StartAbilityCooldown(ab, cd)
                endif
            ...
            if GetUnitAbilityLevel(u, 'A02F') > 0 then
                ...
                set ab = GetUnitAbility(u, 'A02F')
                if ab != null then
                    call ShowAbilityEx(u, 'A02F', true)
                endif
                if cd != 0 then
                    call StartAbilityCooldown(ab, cd)
                endif
            ...
            if LoadInteger(ht, StringHash("specialitem"), GetHandleId(u)) != 0 then
                ...
                set ab = GetUnitAbility(u, 'A00T')
                if ab != null then
                    call ShowAbilityEx(u, 'A00T', true)
                endif
                if cd != 0 then
                    call StartAbilityCooldown(ab, cd)
                endif
    ...
    private function period4 takes nothing returns nothing
        ...
        if count >= 70 and count <= 100 then
            ...
            set h = GetEffectZ(e) + GetAxisZ(GetEffectX(e) + 0.01, GetEffectY(e)) - GetAxisZ(GetEffectX(e), GetEffectY(e))
            if dist > 50 then
                call SetEffectXY(e, x, y, 0)
            endif
            call SetSpecialEffectZ(e, h - 20)
            call Effect("war3mapimported\\stingroar.mdx", x, y, GetEffectZ(e) - GetAxisZ(GetEffectX(e), GetEffectY(e)), a, 0, 0, 0.1 + (count - 70) * 0.05, 1.5, 255, 255, 255, 255)
        endif
        if count == 90 then 
    ...
    private function period5 takes nothing returns nothing
        ...
        if count <= 100 then 
            ...
            loop
                ...
                call SetUnitMoveSpeed(gUnit, GetUnitDefaultMoveSpeed(gUnit) * 0.3)
            endloop
            call GroupClear(gGroup)
        endif
        if count == 100 then 
        ...
        if count >= 100 and count <= 150 then
            ...
            loop
                ...
                exitwhen gUnit == null
                if FilterGeneral(u, gUnit) and not IsUnitInGroup(gUnit, g) then
                    call DamageUnit(u, gUnit, (7 + 2 * GetUnitAbilityLevel(u, 'A023')) * GetHeroStr(u, true))
                    call GroupAddUnit(g, gUnit)
                endif
            endloop
            call GroupClear(gGroup)
    ...
    private function period6_2 takes nothing returns nothing
        ...
        local real h = LoadReal(ht, zid, 4) + GetRandomReal(3, 6)
        ...
        if count >= 20 then
            set g = CreateGroup()
            call GroupEnumUnitsInRange(g, GetEffectX(e), GetEffectY(e), 400, null)
            loop
                set v = GroupForEachUnit(g)
                exitwhen v == null
                if FilterGeneral(u, v) then
                    call DamageUnit(u, v, 15 * GetHeroStr(u, true) / 60)
            ...
            call Effect("war3mapimported\\by_wood_bashenan_juqi_1_4.mdx", GetEffectX(e), GetEffectY(e), h, randomAngle(), 0, 0, 1, 1, 255, 255, 255, 255)
    ...
    private function period6_1 takes unit u, real x, real y returns nothing
        ...
        call Effect("war3mapimported\\by_wood_effect_qigongbo_lan.mdx", GetUnitX(u), GetUnitY(u), 50, a, 0, 0, 2, 2, 255, 255, 255, 255)
    ...
    private function period6 takes nothing returns nothing
        ...
        if count >= 70 and count <= 130 then
            ...
            set x2 = PolarX(x1, GetRandomReal(- 400, 400), randomA)
            set y2 = PolarY(y1, GetRandomReal(- 400, 400), randomA)
            call period6_1(u, x2, y2)
        endif
        if count <= 130 then
            call GroupEnumUnitsInRange(gGroup, GetUnitX(u), GetUnitY(u), 2000, null)
            loop
            ...
            loop
                ...
                if FilterGeneral(u, gUnit) then
                    if not IsUnitInGroup(gUnit, g) then
                        ...
                        call DamageUnit(u, gUnit, 9 * GetHeroStr(u, true))
                        call GroupAddUnit(g, gUnit)
                    endif
                endif
            endloop
